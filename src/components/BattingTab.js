...
function statDot(val, avg, good, elite) {
  if (isNaN(val)) return '#b8b8d0';
  if (val >= elite) return '#90d4a8';
  if (val >= good) return '#90c4e8';
  if (val >= avg) return '#b8b8d0';
  return '#e8a0a0';
}
...
function SlashLine({ stats }) {
  ...
  const items = [
    ...
    { label:'OBP', val:stats.obp||'.---', dot:statDot(obp,0.30,0.33,0.370), tip:'On-Base Percentage: how often a batter reaches base. MLB avg ~.320.' },
    { label:'SLG', val:stats.slg||'.---', dot:statDot(slg,0.37,0.44,0.500), tip:'Slugging Percentage: total bases ÷ at-bats. MLB avg ~.400.' },
...
export default function BattingTab({ data }) {
  ...
  return (
    <>
      {[['away', awayTeam, awayBatters, awayTeamStats, '#60a5fa'], ['home', homeTeam, homeBatters, homeTeamStats, '#f472b6']].map(([side, team, batters, stats, color]) => (
        <div key={side} style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:16, padding:16, marginBottom:10 }}>
          ...
          <TeamTable batters={batters} teamStats={stats} color={color} />
        </div>
      ))}
    </>
  );
