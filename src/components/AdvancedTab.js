...
function PitchZoneCanvas({ pitches, size = 160 }) {
  const ref = useRef();
  useEffect(() => {
    const c = ref.current; if (!c) return;
    const ctx = c.getContext('2d');
    const W = size, H = Math.round(size * 1.15), pad = 22;
...
function AnimatedBar({ pct, color }) {
  return (
    <div style={{ flex:1, height:6, background:'rgba(255,255,255,0.08)', borderRadius:3, overflow:'hidden' }}>
      <div style={{ height:'100%', background:color, borderRadius:3, width:`${pct}%`, transition:'width 0.6s cubic-bezier(0.4,0,0.2,1)' }} />
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
        ...
        <span style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginLeft:4 }}>season series ({games.length} games)</span>
      </div>
    ...
    <div className="tab-panel">
      ...
      <div style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:16, padding:16, marginBottom:10 }}>
        ...
        <div style={{ display:'flex', gap:14, marginTop:14, paddingTop:12, borderTop:'0.5px solid rgba(255,255,255,0.08)', flexWrap:'wrap', fontSize:11, color:'rgba(255,255,255,0.3)' }}>
          {[['elite','Above average'],['good','Slightly above'],['avg','League average'],['poor','Below average']].map(([r,lbl])=>(
            <span key={r} style={{ display:'flex', alignItems:'center', gap:5 }}><TrendArrow rating={r} size={12} />{lbl}</span>
          ))}
        </div>
      </div>

      {/* Pitch zone — unique per pitcher */}
      <div style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:16, padding:16, marginBottom:10 }}>
        ...
        <div style={{ fontSize:11, color:'rgba(255,255,255,0.25)', marginBottom:12 }}>Season pitch tendency by location. Filled = swing; outline = no swing; red ring = strikeout. Historic data.</div>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginBottom:12, fontSize:11 }}>
          {[['#3B82F6','Four-seam FB'],['#06D6A0','Slider/Sweeper'],['#A78BFA','Changeup'],['#F472B6','Sinker/Curve']].map(([col,lbl])=>(
            <span key={lbl} style={{ display:'flex', alignItems:'center', gap:5, color:'rgba(255,255,255,0.45)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          {[[awaySP,awayTeam],[homeSP,homeTeam]].map(([sp,team],i) => sp && (
            <div key={i} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
              <div style={{ fontSize:12, fontWeight:500, color:'rgba(255,255,255,0.6)', textAlign:'center' }}>{sp.name}</div>
              <div style={{ fontSize:10, color:'rgba(255,255,255,0.3)' }}>{team.abbr}</div>
              <PitchZoneCanvas pitches={getPitcherZoneData(KNOWN_ARSENALS[sp.name])} size={150} />
            </div>
          ))}
        </div>
      </div>
