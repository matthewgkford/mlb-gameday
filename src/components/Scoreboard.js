...
function TeamLogo({ abbr, size = 56 }) {
  ...
  if (err) return (
    <div style={{ width:size, height:size, borderRadius:'50%', background:'#1e293b', display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*0.25, fontWeight:700, color:'#fff' }}>
      {abbr?.slice(0,2)}
    </div>
  );
...
function WeatherIcon({ condition }) {
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <circle cx="6" cy="7" r="3.5" fill="#EF9F27"/>
      <ellipse cx="10" cy="10" rx="4.5" ry="3" fill="#94a3b8"/>
      <ellipse cx="8" cy="8.5" rx="2.5" ry="2.5" fill="#94a3b8"/>
    </svg>
  );
}
...
function Base({ occupied }) {
  return (
    <div style={{ width:11, height:11, borderRadius:2, background: occupied?'#fbbf24':'rgba(255,255,255,0.12)', transform:'rotate(45deg)', border: occupied?'none':'1px solid rgba(255,255,255,0.18)' }}></div>
  );
}
