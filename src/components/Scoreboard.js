...
export default function Scoreboard({ data, lastUpdated, onBack, isLive }) {
  ...
  return (
    <div style={{ background:'rgba(255,255,255,0.04)', border:'0.5px solid rgba(255,255,255,0.1)', borderRadius:20, padding:'18px 18px 14px', marginBottom:12 }}>
      ...
      <div style={{ textAlign:'center', marginBottom:12 }}>
        {isFinal ? (
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, flexWrap:'wrap' }}>
            <span style={{ background:'#dc2626', borderRadius:6, padding:'2px 8px', fontSize:11, fontWeight:700, color:'#fff', letterSpacing:0.5 }}><span style={{ display:'inline-block', width:6,height:6,borderRadius:'50%',background:'#fff',marginRight:4,animation:'pulse 1.5s infinite' }}></span>LIVE</span>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <div style={{ fontSize:12, color:'rgba(255,255,255,0.4)' }}><div>{outs} out{outs!==1?'s':''}</div><div>{balls}–{strikes}</div></div>
            </div>
          </div>
            ...
            <tbody>
              {[['away',awayTeam.abbr,awayScore,awayHits,awayErrors],['home',homeTeam.abbr,homeScore,homeHits,homeErrors]].map(([side,abbr,r,h,e])=>(
                <tr key={side} style={{ borderTop:'0.5px solid rgba(255,255,255,0.06)' }}>
                  ...
                  {innings.map(inn=>{ const v=inn[side]?.runs; return <td key={inn.num} style={{ padding:'5px 3px', textAlign:'center', color:v>0?'#fff':'rgba(255,255,255,0.25)', fontWeight:v>0?600:400 }}>{v!==undefined?v:'–'}</td>; })}
      ...
      {isLive&&lastUpdated && <div style={{ fontSize:10, color:'rgba(255,255,255,0.2)', textAlign:'right', marginTop:8 }}>Updated {lastUpdated.toLocaleTimeString()}</div>}
    </div>
  );
}
